...
public class HelloApplication extends Application {
	...
	private static int[][] db;// حفظ فيها مجموع الكوست
	private static String[][] Citydp;// اسماء السيتي
	...
	private static void fillTableLabel() {
		...
		str[city.length - 1] += "\t" + db[numCities - 1][numCities - 1] + "" + Citydp[numCities - 1][numCities - 1];// Ending
																													// City
		for (int i = numCities - 2; i >= 0; i--) {
			...
			for (int j = i + 1; j < numCities; j++) {
				if (stage[i] + 1 == stage[numCities - 1]) {
					if (stage[i] != stage[j]) {
						db[i][j] = LI.get(0).getHotelCost() + LI.get(0).getPetrolCost();
						str[i] += " " + db[i][j];// Cities that lead to the Ending City
					}
				} else {
					if (stage[i] + 1 == stage[j]) {
						for (int k = 0; k < LI.size(); k++) {
							db[i][j] = LI.get(k).getHotelCost() + LI.get(k).getPetrolCost();
							str[i] += " " + db[i][j];// Ways that don't lead to the end
							j++;
						}
						j--;
					} else {
						for (int k = j; stage[k] > stage[i]; k--) {
							if (stage[k] != stage[j] && stage[k] + 1 == stage[j]) {
								if (db[i][j] > 0) {// to find optimal solution
									if (db[i][j] > db[i][k] + db[k][j]) {
										Citydp[i][j] = city[k];
									} else if (db[i][j] == db[i][k] + db[k][j]) {
									...
									str[i] += " " + db[i][j] + "" + Citydp[i][j];
								} else {// to find a first solution
									db[i][j] = db[i][k] + db[k][j];
									Citydp[i][j] = city[k];
									str[i] += " " + db[i][j] + "" + Citydp[i][j];// No Duplicant but not final
																					// solution
									String[] array = str[i].split(" ");
									str[i] = "";
									for (int n = 0; n < array.length - 1; n++) {
	...
	public static void fillAnswer() {
		...
		for (int h = 0; h < s.length; h++) {
			...
			if (h == 0) {
				...
				ta.setText(ta.getText() + "              Cost:" + db[0][numCities - 1] + "\n" + "\n");
			} else {
